...
-- plugin repo to use new branches under the `x.0` scheme ift hey eixst with `ppm gh check-stubs-update-pr --no-pr --branch 3.0 --no-commit --staging .`
...
-- Must be performed at the repository root.
-- ppm gh release Linux/*.so MacOS/*.so Windows/*.dll
if ARGS[2] == "gh" and ARGS[3] == "release" then
  ...
  if release then
    ...
    if contents ~= common.read("manifest.json") then
      if not pcall(run_command, "git config --global user.email") then
        ...
        run_command('git config --global user.name "PPM GH Plugin"')
      end
  ...
  if release and changelog and os.getenv("PPM_DISCORD_WEBHOOK") then
    ...
    print(run_command('curl -H "Content-Type:application/json" "' .. os.getenv("PPM_DISCORD_WEBHOOK") ..'" -X POST -d @/tmp/discord'))
  end
  log.action(string.format("Done."))
  os.exit(0)
end

-- runs a remote pragtical, and compiles it.
if ARGS[2] == "gh" and ARGS[3] == "run" then
  ...
  for i = 4, #ARGS do
    ...
    if s then
      ...
      local s, e, owner, repo, branch = url:find("https://github.com/([^/]+)/(pragtical%w*):(.+)")
      if s then
  ...
  if not system.stat(target) then
    ...
    for i = 2, #repos do
      ...
      run_command("build-prag %s", tmp)
    end
    run_command("sh -c 'cd %s && build-prag'", tmp)
    common.rename(tmp, target)
  end
  BINARY = target .. PATHSEP .. "pragtical"
  DATADIR = target .. PATHSEP .. "data"
  ARGS = newArgs
end

-- updates the list of pragticals in the manifest to incluide the specified release
-- usage: ppm gh update-pragticals manifest.json https://github.com/pragtical/pragtical v2.1.7 [suffix for release]
if ARGS[2] == "gh" and ARGS[3] == "update-pragticals" then
  ...
  if not manifest["pragticals"] then manifest["pragticals"] = {} end
  ...
  for i, asset in ipairs(assert(release_info.assets, "no assets found in release")) do
    if asset.name:find("portable") then
      local tag, arch, os = asset.name:match("pragtical%-([^%-]+)%-([%w_]+)%-([%w_]+)%-portable")
  ...
  local pragtical = { version = version, mod_version = MOD_VERSION or LATEST_MOD_VERSION, assets = relevant_assets }
  local existing_pragtical = common.grep(manifest["pragticals"], function(e) return e.version == version end)[1]
  if existing_pragtical then
    log.action(string.format("Updating existing entry for %s...", version))
    for k,v in pairs(pragtical) do existing_pragtical[k] = v end
  else
    log.action(string.format("Adding new entry for %s...", version))
    table.insert(manifest["pragticals"], pragtical)
  end
  table.sort(manifest["pragticals"], function(a,b) return a.version > b.version end)
